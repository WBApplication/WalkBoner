...
public class ProfilePostsAdapter extends RecyclerView.Adapter<ProfilePostsAdapter.ViewHolder> {
    ...
    public ProfilePostsAdapter(Context context, List<Post> data) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        firestore.collection("users").whereEqualTo("userUid", post.getUserUid()).get().addOnSuccessListener(queryDocumentSnapshots -> {
            for (DocumentSnapshot doc : queryDocumentSnapshots.getDocuments()) {
                ...
                Glide.with(context).load(post.getPostImage()).diskCacheStrategy(DiskCacheStrategy.ALL).addListener(new RequestListener<Drawable>() {
                    @Override
